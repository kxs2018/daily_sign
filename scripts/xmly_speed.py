...
def read(cookies):
    ...
    taskIds = {'242', '239', '241', '240', '238', '236', '237', '235', '234'} - set(result["completeList"])
    params = (
        ('userid', str(uid)),
        ('pid', pid),
        ('taskid', taskIds.pop()),
...
def run():
    ...
    for k, v in enumerate(cookiesList):
        ...
        if autoTakeOut and total >= amount:
            ...
            if pay_info and pay_info['name'] and pay_info['accountType'] and pay_info["accountNumber"]:
                ...
                if task_out_res:
                    QYWX_Notify().send(title=title,
                                       content=f"{user_info_res['nickname']} 提现到账户【{pay_info['accountNumber']}】20元成功")
            else:
                QYWX_Notify().send(title=title, content=f"请先手动填写【user_info_res['nickname']】支付宝账号提现一次")
        print("###" * 20)
        print("\n" * 4)
    ...
    if int(_notify_time.split()[0]) == notify_time and int(_notify_time.split()[1]) < 15:
        ...
        QYWX_Notify().send(title, message)
